...
def main():
    # select project
    neptune.set_project('mayu-ot/art-question-answering')
    ...
    # create experiment
    neptune.create_experiment(name="train_dual_model",
                              upload_stdout=True)
    ...
    valset = datasets.factory_VQA('val', options['vqa'], 
                ...
                opt_clevr=options.get('clevr', None),
                opt_vgenome=options.get('vgnome', None))
    testset = datasets.factory_VQA('test', options['vqa'], 
                opt_coco=options.get('coco', None), 
                opt_clevr=options.get('clevr', None),
                opt_vgenome=options.get('vgnome', None))
    val_loader = valset.data_loader(batch_size=options['optim']['batch_size'],
                                    num_workers=1)
    test_loader = testset.data_loader(batch_size=1 if args.beam_search else options['optim']['batch_size'],
                                      num_workers=1)
    print('Done.')
    print('Setting up the model...')

    
    neptune.send_text('train_data_version', md5(trainset.dataset_path))
    neptune.send_text('test_data_version', md5(testset.dataset_path))
    neptune.send_text('val_data_version', md5(valset.dataset_path))
    ...
    if args.resume:
        ...
        model_hash = md5(os.path.join(options['logs']['dir_logs'], args.resume+"_model.pth.tar"))
        neptune.send_text('model_version', model_hash)
    else:
    ...
    if args.evaluate:
        neptune.append_tags(['evaluation'])
        ...
        save_results(evaluate_result, args.start_epoch, testset.split_name(),
    ...
    for epoch in range(args.start_epoch, options['optim']['epochs']):
        ...
        if options['vqa']['trainsplit'] == 'train':
            ...
            acc1, acc5, acc10, loss_q = engine.validate(val_loader, model,
                                                exp_logger, epoch, args.print_freq)
            if (epoch + 1) % options['optim']['eval_epochs'] == 0:
                #print('[epoch {}] evaluation:'.format(epoch))
                evaluate_result = engine.evaluate(val_loader, model, exp_logger, args.print_freq)   #model.module, exp_logger, args.print_freq)
...
def save_results(results, epoch, split_name, dir_logs, dir_vqa, is_testing=True):
    ...
    neptune.send_artifact(path_rslt)
